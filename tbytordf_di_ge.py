...
parser.add_argument('--re_file1', type=str,  default="./re_output/dg/biomarkers", help='输入re结果位置')
parser.add_argument('--re_file2', type=str,  default="./re_output/dg/improper-regulation-linked-to-disease", help='输入re结果位置')
parser.add_argument('--re_file3', type=str,  default="./re_output/dg/overexpression-in-disease", help='输入re结果位置')
...
try:
    ...
    files = pd.read_csv('./new_about/di_ge/test.tsv',sep='\t')


    for i in range(len(results1.iloc[:,0])):
        lit = list([results1.iloc[i,1],results2.iloc[i,1],results3.iloc[i,1]])
        re = max(lit)
        if re>0.5:
            n = lit.index(re)
            if n == 0:
                entitie_relation = [files.iloc[i,2],"biomarkers",files.iloc[i,3],str(re)]
            if n == 1:
                entitie_relation = [files.iloc[i,2],"improper-regulation-linked-to-disease",files.iloc[i,3],str(re)]
            if n == 2:
                entitie_relation = [files.iloc[i,2],"overexpression-in-disease",files.iloc[i,3],str(re)]
    ...
    a2b.to_csv(args.new_paper_dir+'/di_ge_data.tsv', index=False,sep='\t')
