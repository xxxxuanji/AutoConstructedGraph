...
parser.add_argument('--re_file1', type=str,  default="./re_output/gg/activates", help='输入re结果位置')
parser.add_argument('--re_file2', type=str,  default="./re_output/gg/binding", help='输入re结果位置')
parser.add_argument('--re_file3', type=str,  default="./re_output/gg/regulation", help='输入re结果位置')
...
try:
    ...
    files = pd.read_csv('./new_about/ge_ge/test.tsv',sep='\t')
    ...
    for i in range(len(results1.iloc[:,0])):
        ...
        if re>0.5:
            n = lit.index(re)
            if n == 0:
                entitie_relation = [files.iloc[i,2],"activates",files.iloc[i,3],str(re)]
            if n == 1:
                entitie_relation = [files.iloc[i,2],"binding",files.iloc[i,3],str(re)]
            if n == 2:
                entitie_relation = [files.iloc[i,2],"regulation",files.iloc[i,3],str(re)]
    ...
    a2b.to_csv(args.new_paper_dir+'/ge_ge_data.tsv', index=False,sep='\t')
