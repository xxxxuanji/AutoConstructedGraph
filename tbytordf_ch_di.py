...
parser.add_argument('--re_file1', type=str,  default="./re_output/cd/alleviates", help='输入re结果位置')
parser.add_argument('--re_file2', type=str,  default="./re_output/cd/inhibits-cell-growth", help='输入re结果位置')
parser.add_argument('--re_file3', type=str,  default="./re_output/cd/prevents", help='输入re结果位置')
parser.add_argument('--re_file4', type=str,  default="./re_output/cd/role-in-disease-pathogenesis", help='输入re结果位置')
parser.add_argument('--re_file5', type=str,  default="./re_output/cd/side-effect", help='输入re结果位置')
parser.add_argument('--re_file6', type=str,  default="./re_output/cd/treatment", help='输入re结果位置')
...
try:
    ...
    results6 = pd.read_csv(args.re_file6+"/test_results.tsv",sep='\t',header = None)
    files = pd.read_csv('./new_about/ch_di/test.tsv',sep='\t')  

    for i in range(len(results1.iloc[:,0])):
        lit = list([results1.iloc[i,1],results2.iloc[i,1],results3.iloc[i,1],results4.iloc[i,1],results5.iloc[i,1],results6.iloc[i,1]])
        re = max(lit)
        if re>0.5:
            n = lit.index(re)
            if n == 0:
                entitie_relation = [files.iloc[i,2],"alleviates",files.iloc[i,3],str(re)]
            if n == 1:
                entitie_relation = [files.iloc[i,2],"inhibits-cell-growth",files.iloc[i,3],str(re)]
            if n == 2:
                entitie_relation = [files.iloc[i,2],"prevents",files.iloc[i,3],str(re)]
            if n == 3:
                entitie_relation = [files.iloc[i,2],"role-in-disease-pathogenesis",files.iloc[i,3],str(re)]
            if n == 4:
                entitie_relation = [files.iloc[i,2],"side-effect",files.iloc[i,3],str(re)]
            if n == 5:
                entitie_relation = [files.iloc[i,2],"treatment",files.iloc[i,3],str(re)]
    ...
    a2b.to_csv(args.new_paper_dir+'/ch_di_data.tsv', index=False,sep='\t')
